-- ============================================================
-- 071_m4u_intake_fix.sql — fix "column k does not exist". ADDITIVE.
--
-- BUG (present since 024): the duplicate-lead merge used
--   select jsonb_object_agg(k, v) from jsonb_each(...)
-- but jsonb_each returns columns (key, value) — the alias t(k, v)
-- was missing, so EVERY intake call for an already-known phone
-- raised 42703 and returned HTTP 500. New leads were unaffected.
-- Found 2026-08-19 during the FB backfill (151 of 421 leads 500'd).
--
-- ALSO: leads killed by kamalag.com/sesi registration (dead +
-- label 'Registered', see 070) must NEVER be revived by a later
-- webhook/import — Kamal's rule: once registered, nobody calls
-- them. New 'registered_ignored' branch guards that.
-- ============================================================
...
begin
  ...
  if v_lead.id is null then
    ...
    v_merged := coalesce(v_lead.custom_fields,'{}'::jsonb) ||
                coalesce((select jsonb_object_agg(t.k, t.v)
                            from jsonb_each(coalesce(p_custom,'{}'::jsonb)) as t(k, v)
                           where t.v is not null and t.v::text <> '""' and t.v::text <> 'null'), '{}'::jsonb);

    if v_lead.status = 'dead' and v_lead.current_label = 'Registered' then
      -- registered at kamalag.com/sesi → stays uncallable, never revived
      v_result := 'registered_ignored';
      update m4u_leads set custom_fields = v_merged,
        ghl_contact_id = coalesce(ghl_contact_id, p_contact_id)
      where id = v_id;
    elsif v_prop is not null and v_prop = v_lead.property_id then
