...
/* old planner kept for reference — no longer routed */
function PrimaryMY() {
  const { user } = useApp()
  const [price, setPrice] = useState(350000)
  const [units, setUnits] = useState(2)
  const [target, setTarget] = useState(10000)
  /* project values — admin/VP-set; hidden constants for everyone else */
  const renPct = 2, A = 0.73, B = 0.4
  const D = 0.5 * B, E = 0.3 * B
  const share = (D + E) / 3
  const RATES: Record<string, { label: string; pct: number }> = {
    REN: { label: 'Selling REN', pct: renPct },
    L: { label: 'Leader OV', pct: E },
    TL: { label: 'Team Leader OV', pct: D },
    HOT: { label: 'Head of Team OV', pct: B - share },
    TM: { label: 'Team Manager', pct: 0 },
    VP: { label: 'VP OV', pct: A - B - share },
    GVP: { label: 'GVP', pct: A - B - share },
  }
  const pos = user?.careerRank ?? 'REN'
  const mine = RATES[pos] ?? RATES.REN
  const privileged = user?.role === 'master_admin' || user?.role === 'country_admin' || pos === 'VP' || pos === 'GVP'
  const perUnit = (price * mine.pct) / 100 / 1.08
  const total = perUnit * units
  const unitsNeeded = perUnit > 0 ? Math.ceil(target / perUnit) : 0
  const fmt = (v: number) => `RM ${v.toLocaleString('en-MY', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
  const fmt0 = (v: number) => `RM ${Math.round(v).toLocaleString('en-MY')}`
  return (
    <>
      <div className="mb-3 flex items-center gap-2">
        <Chip tone="accent">🔒 Rates set by admin</Chip>
        <Chip>{mine.label} · {mine.pct.toFixed(3)}%</Chip>
      </div>

      {/* interactive deal */}
      <Card className="mb-4 p-4">
        <p className="mb-3 border-l-2 border-accent pl-2 font-display text-sm font-extrabold">🏗 ERINAZ Suites — play with your numbers</p>
        <div className="mb-1 flex justify-between text-xs font-semibold text-muted">
          <span>Unit price</span><b className="text-ink">{fmt0(price)}</b>
        </div>
        <input type="range" min={200000} max={1500000} step={10000} value={price}
          onChange={(e) => setPrice(Number(e.target.value))} className="mb-3 w-full accent-[var(--accent)]" />
        <div className="mb-1 flex justify-between text-xs font-semibold text-muted">
          <span>Units you close</span><b className="text-ink">{units}</b>
        </div>
        <input type="range" min={0} max={20} value={units}
          onChange={(e) => setUnits(Number(e.target.value))} className="w-full accent-[var(--accent)]" />
      </Card>

      {/* my money — live */}
      <div className="hero-user mb-4 p-5">
        <p className="text-[10px] font-bold uppercase tracking-widest text-[#c9c2a8]">My {pos === 'REN' ? 'commission' : 'OV'} · {units} unit{units === 1 ? '' : 's'}</p>
        <p className="gold-text font-display text-4xl font-extrabold">{fmt(total)}</p>
        <p className="mt-1 text-[11px] text-[#c9c2a8]">{fmt(perUnit)} per unit · net after 8% SST</p>
      </div>

      {/* target planner */}
      <Card className="mb-4 p-4">
        <p className="mb-2 border-l-2 border-accent pl-2 font-display text-sm font-extrabold">🎯 Target — how much do you want?</p>
        <div className="mb-3 flex items-center gap-2">
          {[5000, 10000, 20000, 50000].map((t2) => (
            <button key={t2} type="button" onClick={() => setTarget(t2)}
              className={clsx('flex-1 cursor-pointer rounded-full border px-2 py-2 text-[11px] font-extrabold transition-colors duration-200',
                target === t2 ? 'border-accent bg-accent text-on-accent' : 'border-border text-muted hover:text-ink')}>
              {t2 / 1000}k
            </button>
          ))}
          <input type="number" value={target} onChange={(e) => setTarget(Number(e.target.value) || 0)} aria-label="Custom target"
            className="h-9 w-24 rounded-xl border border-border bg-surface px-2 text-right text-xs font-bold outline-none focus:border-accent" />
        </div>
        <div className="rounded-xl bg-accent-soft p-3.5 text-center">
          <p className="text-xs text-muted">To take home <b className="text-ink">{fmt0(target)}</b> you need</p>
          <p className="font-display text-3xl font-extrabold text-accent">{unitsNeeded} unit{unitsNeeded === 1 ? '' : 's'}</p>
          <div className="mx-auto mt-2 h-2 w-full max-w-xs overflow-hidden rounded-full bg-surface2">
            <div className="h-full rounded-full bg-accent transition-all duration-500" style={{ width: `${Math.min(100, (units / Math.max(unitsNeeded, 1)) * 100)}%` }} />
          </div>
          <p className="mt-1.5 text-[11px] font-semibold text-muted">
            {units >= unitsNeeded && unitsNeeded > 0 ? '🎉 Target hit at current units!' : `${Math.max(unitsNeeded - units, 0)} more to go — you got this 🔥`}
          </p>
        </div>
      </Card>

      {/* full worksheet — VP/admin only */}
      {privileged && (
        <Card className="mb-4 overflow-hidden">
          <p className="border-l-2 border-accent p-4 pb-2 pl-4 font-display text-sm font-extrabold">Full worksheet (VP view)</p>
          {([['REN (selling)', renPct], ['L — 30%×B', E], ['TL — 50%×B', D], ['HOT — B − share', B - share], ['VP — (A−B) − share', A - B - share], ['IQI (funds share)', -share]] as [string, number][]).map(([lbl, pct]) => (
            <div key={lbl} className="flex items-center justify-between border-b border-border px-4 py-2.5 text-sm last:border-0">
              <span>{lbl}</span>
              <span className="text-right"><b>{pct.toFixed(3)}%</b> · {fmt((price * pct) / 100 / 1.08)}</span>
            </div>
          ))}
        </Card>
      )}
      <p className="pb-4 text-center text-[10px] text-muted">
        {privileged ? 'VP/admin view — agents see their own rate only' : 'You see your part only — rates set by admin'} · MY rules locked (spec §E)
      </p>
    </>
  )
}
